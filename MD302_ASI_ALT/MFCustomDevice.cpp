...
void MFCustomDevice::attach(uint16_t adrPin, uint16_t adrType, uint16_t adrConfig, bool configFromFlash)
{
    ...
    ********************************************************************************** */
    ...
    if (strcmp(parameter, "MD302_ASI_ALT") == 0)
        _customType = MD302_ASI_ALT_DEVICE_1;

    if (_customType == MD302_ASI_ALT_DEVICE_1) {
        /* **********************************************************************************
            Check if the device fits into the device buffer
        ********************************************************************************** */
        if (!FitInMemory(sizeof(MD302_ASI_ALT))) {
        ...
        ********************************************************************************** */
        ...
        _myASI_ALT_device = new (allocateMemory(sizeof(MD302_ASI_ALT))) MD302_ASI_ALT(_pin1, _pin2);
        _myASI_ALT_device->attach(Parameter1, Parameter2);
...
void MFCustomDevice::set(int16_t messageID, char *setPoint)
{
    ...
    if (_customType == MD302_ASI_ALT_DEVICE_1) {
        _myASI_ALT_device->set(messageID, setPoint);
    }
}